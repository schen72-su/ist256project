...
#the function to download the url page - Sirui, Richard
def download(url,i_headers,retries_nums,parm = {}):
    html_doc = ""
    try:
        #setting up the URL transmission parameters -Sirui
        if len(parm) == 0:
        ...
        html_doc = response.read()  # web scraping -Richard
        wait_seconds()
    except  Exception as  ex:
        print("Download error:",ex)
        if retries_nums > 0 :
            #If download fails, try it again -Richard
            html_doc = download(url, i_headers, retries_nums - 1,parm)
            wait_seconds()
    return html_doc

#delay function -Xinyan
def wait_seconds():
    time.sleep(random.randint(3,5))

#looking up the coordinates for the inputted location. -Xinyan
...
#taking the coordinates from google geocode api and loading the temperature through darksky weather. -Sirui
...
#Find suitable clothing according to local weather and save pictures locally -Richard
def save_cloth_imgs(type, filename):
    ...
    parms = {'q':type}
    #Find the corresponding photo gallery, with the key values we set up, we search HM for random images depending on the img. - Richard
    ...
    human_img_url_list = []
    for div in img_divs:
        # print(str(div.find(name='div',attrs={'class':'image'}).find_all(name='img')[1]['src'])) -SIrui
...
#converting farenheight to celcius -Xinyan
def F_to_C(tem):
    return  round((tem - 32)/1.8 ,1)


#if else statement. Printing out the different temperature type depending on the numerical temperature. -Xinyan
